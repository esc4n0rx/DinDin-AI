...
/**
 * Verifica se um usuário está no fluxo de criação de metas
 * @param {string|number} telegramId - ID do usuário no Telegram
 * @returns {boolean} - Retorna true se o usuário está no fluxo de criação de metas
 */
function isUserInGoalFlow(telegramId) {
  return goalConversations.has(telegramId);
}

/**
 * Processa mensagens de usuários que estão no fluxo de criação de metas
 * @param {TelegramBot} bot - Instância do bot do Telegram
 * @param {Object} msg - Objeto da mensagem do Telegram
 */
async function handleOngoingGoalFlow(bot, msg) {
  const { id: telegramId, first_name } = msg.from;
  const chatId = msg.chat.id;
  const text = msg.text;
  
  console.log(`Processando mensagem do fluxo de meta: "${text}" de ${telegramId}`);
  ...
  try {
    ...
    // Verificar o estado atual da conversa
    const conversation = goalConversations.get(telegramId);
    if (!conversation) {
      console.log(`Erro: Não encontrou conversa para usuário ${telegramId}`);
      return;
    }
    
    console.log(`Estado atual da conversa: ${conversation.state}`);
    ...
    switch (conversation.state) {
      case 'awaiting_title':
        ...
        await bot.sendMessage(chatId, targetPrompt, { parse_mode: 'Markdown' });
        break;
        
      case 'awaiting_target_amount':
        // Processar valor alvo
        try {
          const numericValue = text.replace(/[R$\s]/g, '').replace(',', '.');
          ...
          await bot.sendMessage(chatId, initialPrompt, { parse_mode: 'Markdown' });
        } catch (error) {
      ...
      case 'awaiting_initial_amount':
        ...
        try {
          ...
          await bot.sendMessage(chatId, datePrompt, { parse_mode: 'Markdown' });
        } catch (error) {
      ...
      case 'awaiting_target_date':
        ...
        try {
          ...
          if (!/^(não|nao|n|no|sem prazo|sem data|indefinido)$/i.test(text)) {
            // Tentar extrair data
            if (/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(text)) {
              // Formato DD/MM/YYYY
              targetDate = moment(text, 'DD/MM/YYYY').toDate();
            } else if (/^\d{4}-\d{1,2}-\d{1,2}$/.test(text)) {
              // Formato YYYY-MM-DD
              targetDate = moment(text, 'YYYY-MM-DD').toDate();
            } else {
              // Outros formatos
      ...
      default:
        console.log(`Estado desconhecido: ${conversation.state}`);
        break;
    }
  } catch (error) {
    console.error('Erro no handleOngoingGoalFlow:', error);
    await bot.sendMessage(
      chatId,
  ...
  try {
    ...
    switch (action) {
      case 'goal_add': {
        ...
        goalConversations.set(telegramId, {
          state: 'awaiting_contribution_amount',
          targetGoal: goal
        });
